...
/* Move output stuffers to input for decrypt tests */
...
int main(int argc, char **argv)
{
    ...
    struct s2n_blob mac_key = {.data = mac_key_sha,.size = sizeof(mac_key_sha) };
    ...
    for (int j = 0; j < 3; j++ ) {
        for (int i = 0; i < max_aligned_fragment; i++) {
            ...
            const uint16_t mac_size = SHA_DIGEST_LENGTH;
