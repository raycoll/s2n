...
int main(int argc, char **argv)
{
    ...
    conn->initial.cipher_suite->active_record_alg = &s2n_record_alg_aes128_sha_composite;
    ...
    for (int j = 0; j < 3; j++ ) {
        for (int i = 0; i < max_aligned_fragment; i++) {
            ...
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->set_encryption_key(&conn->initial.server_key, &aes128));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->set_decryption_key(&conn->initial.client_key, &aes128));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->io.comp.set_mac_write_key(&conn->initial.server_key, mac_key_sha, sizeof(mac_key_sha)));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->io.comp.set_mac_write_key(&conn->initial.client_key, mac_key_sha, sizeof(mac_key_sha)));
    ...
    conn->initial.cipher_suite->active_record_alg = &s2n_record_alg_aes256_sha_composite;
    for (int j = 0; j < 3; j++ ) {
        for (int i = 0; i < max_aligned_fragment; i++) {
            struct s2n_blob in = {.data = random_data,.size = i };
            int bytes_written;
            ...
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->set_encryption_key(&conn->initial.server_key, &aes256));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->set_decryption_key(&conn->initial.client_key, &aes256));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->io.comp.set_mac_write_key(&conn->initial.server_key, mac_key_sha, sizeof(mac_key_sha)));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->io.comp.set_mac_write_key(&conn->initial.client_key, mac_key_sha, sizeof(mac_key_sha)));
    ...
    conn->initial.cipher_suite->active_record_alg = &s2n_record_alg_aes128_sha256_composite;
    for (int j = 0; j < 3; j++ ) {
        for (int i = 0; i < max_aligned_fragment; i++) {
            struct s2n_blob in = {.data = random_data,.size = i };
            int bytes_written;
            ...
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->set_encryption_key(&conn->initial.server_key, &aes128));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->set_decryption_key(&conn->initial.client_key, &aes128));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->io.comp.set_mac_write_key(&conn->initial.server_key, mac_key_sha256, sizeof(mac_key_sha256)));
            EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->io.comp.set_mac_write_key(&conn->initial.client_key, mac_key_sha256, sizeof(mac_key_sha256)));
    ...
    conn->initial.cipher_suite->active_record_alg = &s2n_record_alg_aes256_sha256_composite;
    for (int j = 0; j < 3; j++ ) {
        for (int i = 0; i < max_aligned_fragment; i++) {
            struct s2n_blob in = {.data = random_data,.size = i };
            int bytes_written;
