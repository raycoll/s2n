...
{
    GUARD(server_conn->initial.cipher_suite->active_record_alg->cipher->init(&server_conn->initial.server_key));
    GUARD(server_conn->initial.cipher_suite->active_record_alg->cipher->init(&server_conn->initial.client_key));
    GUARD(server_conn->initial.cipher_suite->active_record_alg->cipher->set_encryption_key(&server_conn->initial.server_key, key));
    GUARD(server_conn->initial.cipher_suite->active_record_alg->cipher->set_decryption_key(&server_conn->initial.client_key, key));
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < max_fragment; i++) {
        ...
        predicted_length += conn->initial.cipher_suite->active_record_alg->cipher->io.aead.record_iv_size;
        predicted_length += conn->initial.cipher_suite->active_record_alg->cipher->io.aead.tag_size;
    ...
    EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->destroy_key(&conn->initial.server_key));
    EXPECT_SUCCESS(conn->initial.cipher_suite->active_record_alg->cipher->destroy_key(&conn->initial.client_key));
