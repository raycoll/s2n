...
static uint16_t overhead(struct s2n_connection *conn)
{
    ...
    uint16_t extra = s2n_hmac_digest_size(active->cipher_suite->active_record_alg->hmac_alg);

    if (active->cipher_suite->active_record_alg->cipher->type == S2N_CBC) {
        /* Subtract one for the padding length byte */
        extra += 1;

        if (conn->actual_protocol_version > S2N_TLS10) {
            extra += active->cipher_suite->active_record_alg->cipher->io.cbc.record_iv_size;
        }
    } else if (active->cipher_suite->active_record_alg->cipher->type == S2N_AEAD) {
        extra += active->cipher_suite->active_record_alg->cipher->io.aead.tag_size;
        extra += active->cipher_suite->active_record_alg->cipher->io.aead.record_iv_size;
    } else if (active->cipher_suite->active_record_alg->cipher->type == S2N_COMPOSITE && conn->actual_protocol_version > S2N_TLS10) {
        extra += active->cipher_suite->active_record_alg->cipher->io.comp.record_iv_size;
    }
...
int s2n_record_max_write_payload_size(struct s2n_connection *conn)
{
    ...
    if (active->cipher_suite->active_record_alg->cipher->type == S2N_CBC) {
        max_fragment_size -= max_fragment_size % active->cipher_suite->active_record_alg->cipher->io.cbc.block_size;
    } else if (active->cipher_suite->active_record_alg->cipher->type == S2N_COMPOSITE) {
        max_fragment_size -= max_fragment_size % active->cipher_suite->active_record_alg->cipher->io.comp.block_size;
        /* Composite digest length */
        max_fragment_size -= active->cipher_suite->active_record_alg->cipher->io.comp.mac_key_size;
        /* Padding length byte */
        max_fragment_size -= 1;
    }
...
int s2n_record_write(struct s2n_connection *conn, uint8_t content_type, struct s2n_blob *in)
{
    ...
    if (cipher_suite->active_record_alg->cipher->type == S2N_CBC) {
        block_size = cipher_suite->active_record_alg->cipher->io.cbc.block_size;
        ...
        block_size = cipher_suite->active_record_alg->cipher->io.comp.block_size;
    }
    ...
    } else if (cipher_suite->active_record_alg->cipher->type == S2N_CBC || cipher_suite->active_record_alg->cipher->type == S2N_COMPOSITE) {
        iv.size = block_size;
        iv.data = implicit_iv;
	...
	case S2N_AEAD:
		GUARD(s2n_stuffer_skip_write(&conn->out, cipher_suite->active_record_alg->cipher->io.aead.record_iv_size));
		encrypted_length += cipher_suite->active_record_alg->cipher->io.aead.tag_size;
		break;
	case S2N_CBC:
		if (conn->actual_protocol_version > S2N_TLS10) {
    ...
    switch (cipher_suite->active_record_alg->cipher->type) {
    case S2N_STREAM:
        GUARD(cipher_suite->active_record_alg->cipher->io.stream.encrypt(session_key, &en, &en));
        break;
    case S2N_CBC:
        GUARD(cipher_suite->active_record_alg->cipher->io.cbc.encrypt(session_key, &iv, &en, &en));
        ...
        break;
    case S2N_AEAD:
        GUARD(cipher_suite->active_record_alg->cipher->io.aead.encrypt(session_key, &iv, &aad, &en, &en));
        break;
    case S2N_COMPOSITE:
        /* This will: compute mac, append padding, append padding length, and encrypt */
        GUARD(cipher_suite->active_record_alg->cipher->io.comp.encrypt(session_key, &iv, &en, &en));
