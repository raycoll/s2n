...
int s2n_record_parse(struct s2n_connection *conn)
{
    ...
    if (cipher_suite->active_record_alg->cipher->type == S2N_CBC) {
        iv.data = implicit_iv;
        iv.size = cipher_suite->active_record_alg->cipher->io.cbc.record_iv_size;
        lte_check(cipher_suite->active_record_alg->cipher->io.cbc.record_iv_size, S2N_TLS_MAX_IV_LEN);
    ...
    } else if (cipher_suite->active_record_alg->cipher->type == S2N_COMPOSITE) {
        /* Don't reduce encrypted length for explicit IV, composite decrypt expects it */
        iv.data = implicit_iv;
        iv.size = cipher_suite->active_record_alg->cipher->io.comp.record_iv_size;
    }
     ...
     */
    if (cipher_suite->active_record_alg->cipher->type == S2N_COMPOSITE) {
        ...
        int mac_size;
        GUARD(cipher_suite->active_record_alg->cipher->io.comp.initial_hmac(session_key, sequence_number, content_type, conn->actual_protocol_version,
                                                         payload_length, &mac_size));

        payload_length -= mac_size;
        /* Adjust payload_length for explicit IV */
        if (conn->actual_protocol_version > S2N_TLS10) {
            payload_length -= cipher_suite->active_record_alg->cipher->io.comp.record_iv_size;
        }
    }

    /* In AEAD mode, the explicit IV is in the record */
    if (cipher_suite->active_record_alg->cipher->type == S2N_AEAD) {
        gte_check(en.size, cipher_suite->active_record_alg->cipher->io.aead.record_iv_size);
        ...
        GUARD(s2n_stuffer_write_bytes(&iv_stuffer, implicit_iv, cipher_suite->active_record_alg->cipher->io.aead.fixed_iv_size));
        GUARD(s2n_stuffer_write_bytes(&iv_stuffer, en.data, cipher_suite->active_record_alg->cipher->io.aead.record_iv_size));
        ...
        gte_check(payload_length, cipher_suite->active_record_alg->cipher->io.aead.record_iv_size + cipher_suite->active_record_alg->cipher->io.aead.tag_size);
        payload_length -= cipher_suite->active_record_alg->cipher->io.aead.record_iv_size;
        payload_length -= cipher_suite->active_record_alg->cipher->io.aead.tag_size;
    ...
    /* Decrypt stuff! */
    switch (cipher_suite->active_record_alg->cipher->type) {
    case S2N_STREAM:
        GUARD(cipher_suite->active_record_alg->cipher->io.stream.decrypt(session_key, &en, &en));
        break;
    case S2N_CBC:
        ...
        GUARD(cipher_suite->active_record_alg->cipher->io.cbc.decrypt(session_key, &iv, &en, &en));
    ...
    case S2N_AEAD:
        ...
        en.size -= cipher_suite->active_record_alg->cipher->io.aead.record_iv_size;
        en.data += cipher_suite->active_record_alg->cipher->io.aead.record_iv_size;

        /* Check that we have some data to decrypt */
        ne_check(en.size, 0);

        GUARD(cipher_suite->active_record_alg->cipher->io.aead.decrypt(session_key, &iv, &aad, &en, &en));
        break;
    case S2N_COMPOSITE:
        ne_check(en.size, 0);
        eq_check(en.size % iv.size,  0);
        ...
        GUARD((cipher_suite->active_record_alg->cipher->io.comp.decrypt(session_key, &iv, &en, &en)));
    ...
    if (cipher_suite->active_record_alg->cipher->type == S2N_CBC || cipher_suite->active_record_alg->cipher->type == S2N_COMPOSITE) {
        gt_check(en.size, 0);
    ...
    /* Skip the IV, if any */
    if (cipher_suite->active_record_alg->cipher->type == S2N_CBC && conn->actual_protocol_version > S2N_TLS10) {
        GUARD(s2n_stuffer_skip_read(&conn->in, cipher_suite->active_record_alg->cipher->io.cbc.record_iv_size));
    } else if (cipher_suite->active_record_alg->cipher->type == S2N_AEAD && conn->actual_protocol_version >= S2N_TLS12) {
        GUARD(s2n_stuffer_skip_read(&conn->in, cipher_suite->active_record_alg->cipher->io.aead.record_iv_size));
    } else if (cipher_suite->active_record_alg->cipher->type == S2N_COMPOSITE && conn->actual_protocol_version > S2N_TLS10) {
        GUARD(s2n_stuffer_skip_read(&conn->in, cipher_suite->active_record_alg->cipher->io.comp.record_iv_size));
    }
