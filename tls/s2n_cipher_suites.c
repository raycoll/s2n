...
struct s2n_cipher_suite s2n_null_cipher_suite = {
    ...
    .active_record_alg = &s2n_record_alg_null,
...
struct s2n_cipher_suite s2n_rsa_with_aes_256_cbc_sha256 = /* 0x00,0x3D */ {
    ...
    .key_exchange_alg = &s2n_rsa,
    .active_record_alg = NULL,
    .all_record_algs = { &s2n_record_alg_aes256_sha256, &s2n_record_alg_aes256_sha256 },
    .num_record_algs = 2,
    .tls12_prf_alg = S2N_HMAC_SHA256,
    .minimum_required_tls_version = S2N_TLS12,
};
...
int s2n_cipher_suites_init(void)
{
    ...
    for (int i = 0; i < num_cipher_suites; i++) {
        ...
        cur_suite->active_record_alg = NULL;

        /* Find the highest priority supported record algorithm */
        for (int j = 0; j < cur_suite->num_record_algs; j++) {
            /* Check if we can use the record algorithm's cipher. Won't be available if the system CPU architechure
             * doesn't support it or if the libcrypto is incompatible. We don't check hmac_alg as all of our
             * implmentations should be supported.
             */
            if (cur_suite->all_record_algs[j]->cipher->is_available()) {
                /* Found a supported record algorithm. We can use the cipher suite */
                cur_suite->available = 1;
                cur_suite->active_record_alg = cur_suite->all_record_algs[j];
                break;
            }
        }
    }
