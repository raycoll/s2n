...
{
    if (conn->handshake_params.num_matching_certs > 0) {
        return s2n_get_compatible_cert_chain_and_key(conn->handshake_params.matching_certs, conn->handshake_params.num_matching_certs, cipher_suite);
    } else {
...
static int s2n_set_cipher_as_server(struct s2n_connection *conn, uint8_t * wire, uint32_t count, uint32_t cipher_suite_len)
{
    ...
    if (higher_vers_match) {
        ...
        conn->handshake_params.our_chain_and_key = s2n_conn_get_compatible_cert_chain_and_key(conn, higher_vers_match);
        return 0;
    }
