...
static int s2n_connection_wipe_keys(struct s2n_connection *conn)
{
     ...
     * keys are allocated. */
    if (conn->secure.cipher_suite && conn->secure.cipher_suite->active_record_alg->cipher->destroy_key) {
        GUARD(conn->secure.cipher_suite->active_record_alg->cipher->destroy_key(&conn->secure.client_key));
        GUARD(conn->secure.cipher_suite->active_record_alg->cipher->destroy_key(&conn->secure.server_key));
    }
