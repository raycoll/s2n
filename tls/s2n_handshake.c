...
int s2n_conn_find_name_matching_certs(struct s2n_connection *conn)
{
    ...
    for (int i = 0; i < conn->config->num_certificates; i++) {
        ...
        if (s2n_cert_chain_and_key_matches_name(chain_and_key, name)) {
            /* Found a match, add it to the list for this session. Yes, this needs to be addressed when the list
             * becomes large.
             */
            conn->handshake_params.matching_certs[conn->handshake_params.num_matching_certs] = chain_and_key;
            conn->handshake_params.num_matching_certs++;
        }
    }
