...
static int handshake_write_io(struct s2n_connection *conn)
{
    ...
    while (s2n_stuffer_data_available(&conn->handshake.io) > 0) {
        ...
        if (conn->mode == S2N_CLIENT) {
            GUARD(conn->client->cipher_suite->active_record_alg->record_write(conn, record_type, &out));
        } else {
            GUARD(conn->server->cipher_suite->active_record_alg->record_write(conn, record_type, &out));
        }
