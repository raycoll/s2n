...
/* (cipher,hmac) algorithm tuple used for per-record security */
...
struct s2n_cipher_suite {
    /* Is the cipher suite implementation available? Set in s2n_cipher_suites_init() and determined by
    ...
    /* Algorithms used for per-record security. active_record_alg is set in s2n_cipher_suite_init */
    const struct s2n_record_algorithm *active_record_alg;

    /* List of record alg implementations sorted by priority. Selection is based on hardware and libcrypto
     * capabilities.
     */
